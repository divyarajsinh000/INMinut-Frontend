...
export type ShareNewsPayload = Pick<NewsItem, 'title' | 'description' | 'content' | 'media' | 'hashtags' | 'cities' | 'publishedDate'> & { link?: string };
...
const cleanFileName = (value?: string, fallback = 'media') => {
  const rawName = (value || fallback).split(/[\\/]/).pop() || fallback;

  let normalized = rawName;
  try {
    normalized = decodeURIComponent(rawName);
  } catch {
    normalized = rawName;
  }
  if (/[ÃÂÊËÎÏÔÛâãäåçèéêëìíîïñòóôõöùúûüýþÿÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞß]/.test(normalized)) {
    try {
      const latin1Bytes = Uint8Array.from(
        [...normalized].map((char) => char.charCodeAt(0) & 0xff),
      );
      normalized = new TextDecoder('utf-8').decode(latin1Bytes);
    } catch {
      // ignore and fall through
    }
  }
  const kept = normalized.replace(/[^\p{L}\p{N}._ -]/gu, '_');
  return kept.trim() || fallback;
};
...
const buildNewsPdfHtml = (payload: ShareNewsPayload) => {
  ...
  return `
    ...
    <html>
      ...
      <body>
        <div class="header">
          <div class="brand">INMinut News</div>
