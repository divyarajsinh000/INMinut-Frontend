...
interface NewsCardProps {
  item: NewsItem;
  onMediaLayout?: (layout: { x: number; y: number; width: number; height: number }) => void;
  onActionsLayout?: (layout: { x: number; y: number; width: number; height: number }) => void;
}
...
export default function NewsCard({ item, onMediaLayout, onActionsLayout }: NewsCardProps) {
  ...
  const titleColor = isDark ? '#FFFFFF' : normalizeHexColor(item.titleColor, AppPalette.navyBlue);
  ...
  const displayText = (() => {
    ...
    const truncatedFifth = fifthLineText.substring(0, Math.max(0, fifthLineText.length - 22));
  ...
  const shareFirstImageAsBrandedPng = async () => {
    ...
    const firstImageUrl = hasImage ? getMediaUrl(images[0]?.url) : 'breaking_placeholder';
  ...
  return (
    <View style={[styles.container, isDark && { backgroundColor: '#1E293B', borderColor: '#334155' }]}>
      {images.length > 0 ? (
        <View ref={mediaRef} onLayout={handleMediaLayout} style={styles.imageWrap}>
          <ScrollView
            horizontal
            pagingEnabled
          ...
          {item.isBreaking && (
            <Animated.View style={[styles.breakingBadge, { backgroundColor: item.breakingBgColor || '#EF4444', opacity: blinkAnim }]}>
              <Ionicons name="flash" size={13} color={item.breakingTextColor || '#FFFFFF'} />
              <Text style={[styles.breakingText, { color: item.breakingTextColor || '#FFFFFF' }]}>{breakingLabel}</Text>
            </Animated.View>
          )}
      ...
      <View style={styles.content}>
        ...
        {item.isBreaking && images.length === 0 && (
          <Animated.View style={[styles.inlineBreakingBadge, { backgroundColor: item.breakingBgColor || '#EF4444', opacity: blinkAnim }]}>
            <Ionicons name="flash" size={13} color={item.breakingTextColor || '#FFFFFF'} />
            <Text style={[styles.breakingText, { color: item.breakingTextColor || '#FFFFFF' }]}>{breakingLabel}</Text>
          </Animated.View>
        )}
    ...
    {descriptionExpanded ? (
      <>
        ...
        >
          ...
          <Ionicons name="chevron-up" size={15} color={AppPalette.brightOrange} />
        </TouchableOpacity>
      </>
    ) : (
      <Pressable onPress={() => renderedLines.length > 5 && setDescriptionExpanded(true)}>
        <Text
          style={[styles.description, { fontSize: descriptionFontSize, lineHeight: Math.round(descriptionFontSize * 1.45) }, isDark && { color: '#FFFFFF' }]}
        ...
        {hashtags.length > 0 && (
          <View style={styles.hashtagsContainer}>
            {hashtags.slice(0, descriptionExpanded ? hashtags.length : 3).map((tag, index) => (
          ...
          {!!(item.reporter?.name?.trim() || item.reporter?.avatar?.trim()) && (
            <View style={styles.reporterContainer}>
              {!!item.reporter?.avatar?.trim() && (
                <Image source={{ uri: getMediaUrl(item.reporter.avatar) }} style={styles.reporterAvatar} contentFit="cover" />
              )}
              {!!item.reporter?.name?.trim() && (
                <Text style={[styles.reporterName, isDark && { color: '#F8FAFC' }]} numberOfLines={1}>{item.reporter.name}</Text>
              )}
            </View>
          )}

          <View ref={actionsRef} onLayout={handleActionsLayout} style={styles.actionsContainer}>
            <TouchableOpacity onPress={handleShare} style={[styles.actionButton, isDark && { backgroundColor: '#334155', borderColor: '#475569' }]} activeOpacity={0.8} disabled={sharing}>
              {sharing ? <ActivityIndicator size="small" color={isDark ? '#F8FAFC' : AppPalette.ink} /> : <Ionicons name="share-social-outline" size={19} color={isDark ? '#F8FAFC' : AppPalette.ink} />}
            </TouchableOpacity>
            <TouchableOpacity onPress={() => toggleSavedNews(item._id)} style={[styles.actionButton, isDark && { backgroundColor: '#334155', borderColor: '#475569' }]} activeOpacity={0.8}>
              <Ionicons name={isSaved ? 'bookmark' : 'bookmark-outline'} size={19} color={isSaved ? Colors.brightOrange : (isDark ? '#F8FAFC' : AppPalette.ink)} />
            </TouchableOpacity>
          </View>
        </View>
      </View>
      ...
      {shareImageUri && (
        <View pointerEvents="none" style={styles.hiddenShareCapture}>
          <View ref={brandedShareRef} collapsable={false} style={{ alignSelf: 'flex-start' }}>
            <BrandedShareImage
